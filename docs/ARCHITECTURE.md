...
| Ledger (Hedera) | **Stubbed by default, verified working end-to-end with real credentials** | `LedgerClient` interface with a `StubLedgerClient` (local, deterministic, fully testable) and a complete `HederaSidecarClient` + Node.js sidecar (`ledger-sidecar/`). Flip `LEDGER_BACKEND=hedera_sidecar` in `backend/.env` once `ledger-sidecar/.env` has real values from the [Hedera Portal faucet](https://portal.hedera.com/) and the topic has been created (`npm run create-topic` in `ledger-sidecar/`, once). Confirmed live: with real testnet credentials, a submitted report anchors to an actual Hedera Consensus Service topic, is independently visible via the public mirror node (`testnet.mirrornode.hedera.com`) within seconds, and `ledger_verified` in the audit-trail API recomputes true against it — not just against the stub. |
